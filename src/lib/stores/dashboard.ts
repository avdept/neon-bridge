...
class DashboardStore {
  ...
  async updateDashboard(updates: Partial<Dashboard>): Promise<boolean> {
    try {
      const current = await this.getCurrentDashboard();
      if (!current) {
        console.error('No current dashboard to update');
        return false;
      }

      const updatedDashboard = await dashboardAPI.updateDashboard(current.id, updates);

      currentDashboard.set(updatedDashboard);

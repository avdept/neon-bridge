<script lang="ts">
  ...
  const title = $derived(config?.title || "Radarr");
  ...
  const healthAlerts = $derived(() => {
    if (!stats.healthAlerts || !Array.isArray(stats.healthAlerts) || stats.healthAlerts.length === 0) {
      return []; // Return original alert(s) if no health alerts
    }
    ...
    // Add original alert if it exists and we have health alerts
  ...
  <div class="radarr-widget">
    ...
    {:else}
      ...
      <StatsGrid columns={3}>
        ...
        />
        <Stat label="Missing" value={formatNumber(stats.missingMovies || 0)} />
        <Stat

...
export interface PluginConfigField {
  ...
  description?: string;
  credential?: boolean; // Indicates if the field is a credential (e.g., password, token) and will not be saved or sent to frontend
  placeholder?: string;
}
...
export interface Plugin {
  ...
  fetchData?: (config: PluginConfig, widgetId?: string | number) => Promise<PluginData>;
...
// Helper function to get standardized refresh rate from plugin config
export function getPluginRefreshRate(config: PluginConfig): number {
  const rate = config.refreshRate || 30;
  // Ensure rate is within reasonable bounds (10-300 seconds)
  return Math.max(10, Math.min(300, rate));
}

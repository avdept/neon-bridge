<script lang="ts">
  ...
  function handleConfigurationError(event: Event) {
    const customEvent = event as CustomEvent;
    isTestingConfig = false;
    dispatch("testing", { testing: false });
    configError = customEvent.detail.error;
  }

  function handleConfigurationSuccess(event: Event) {
    isTestingConfig = false;
    dispatch("testing", { testing: false });
    configError = null;
  }
  ...
  onMount(() => {
    document.addEventListener(
      "configuration-error",
      handleConfigurationError as EventListener
    );
    document.addEventListener(
      "configuration-success",
      handleConfigurationSuccess as EventListener
    );
  });
  ...
  $: {
    ...
    if (plugin) {
      // Initialize config with initial values if provided, otherwise use defaults
      config = {};
  ...
  function handleSubmit() {
    ...
    configError = null;
    dispatch("testing", { testing: true });
    dispatch("submit", { pluginId: selectedPluginId, config });
  }

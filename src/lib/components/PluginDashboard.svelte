<script lang="ts">
  ...
  // Listen for tooltip events from cards
  ...
  // Get the actual instances array
  const instancesArray = $derived(instances());

  // Create render items with gap fillers
  ...
  $effect(() => {
    // Calculate placeholders based on existing widgets
    if (instancesArray.length === 0) {
      placeholders = [];
      return;
    }
    ...
    // Calculate total positions used by all render items (including gaps)
    ...
    renderItems.forEach((item) => {
      if (item.type === "widget" && item.instance) {
        ...
        totalPositionsUsed += 1; // Gaps take 1 position
      }
    });
    ...
    // Add placeholders to fill current row (if not complete)
    ...
    // Add full new row of placeholders
  ...
  // Debug: Log the render data
  $effect(() => {
    console.log("PluginDashboard instances:", instances);
    console.log("PluginDashboard renderItems with gaps:", renderItems);
    console.log("PluginDashboard placeholders:", placeholders);
  });
</script>

<script lang="ts">
  ...
  // If in edit mode, start on step 2 and pre-select plugin
  ...
  function handlePluginSelect(event: CustomEvent) {
    selectedPluginId = event.detail.pluginId;
  }

  function nextStep() {
    if (currentStep === 1 && selectedPluginId) {
      currentStep = 2;
      dispatch("step-change", { step: 2, pluginId: selectedPluginId });
    }
  }
  ...
  function handleConfigSubmit(event: CustomEvent) {
    ...
    dispatch("save", { pluginId, config });
  }

  function handleTestingChange(event: CustomEvent) {
    isTestingConfig = event.detail.testing;
  }
  ...
  function handleRemove() {
    if (
      ...
      dispatch("remove", { pluginId: selectedPluginId });
    }
  }
</script>
...
<div class="add-plugin-form">
  ...
  <div class="step-content">
    {#if currentStep === 1}
      <PluginSelector bind:selectedPluginId on:select={handlePluginSelect} />
    {:else if currentStep === 2 && selectedPluginId}
      <PluginConfigForm
        {selectedPluginId}
        {initialConfig}
        on:submit={handleConfigSubmit}
        on:testing={handleTestingChange}
      />
    {/if}
  </div>

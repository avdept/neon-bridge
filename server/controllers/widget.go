...
func GetWidgets(c *gin.Context) {
	...
	result := DB.Where("dashboard_id = ?", dashboardID).Find(&widgets)
	if result.Error != nil {
...
func GetWidget(c *gin.Context) {
	...
	result := DB.Preload("Dashboard").First(&widget, id)
	if result.Error != nil {
...
func CreateWidget(c *gin.Context) {
	...
	if err := DB.First(&dashboard, widget.DashboardID).Error; err != nil {
...
func UpdateWidget(c *gin.Context) {
	...
	var widget models.Widget
	if err := DB.First(&widget, id).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Widget not found"})
		return
	}
